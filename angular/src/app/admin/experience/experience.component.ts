...
export class ExperienceComponent implements OnInit, OnDestroy {
  ...
  displayedColumns = ['select',  'period', 'societe', 'job', 'task', 'tech', 'links', 'option'];
...
this.societe.value === '' ? '*' : this.societe.value,
this.job.value === '' ? '*' : this.job.value,
this.task.value === '' ? '*' : this.task.value,
this.tech.value === '' ? '*' : this.tech.value,
this.links.value === '' ? '*' : this.links.value,
...
this.links.setValue('');
  ...
  add() {
    this.openDialog(new Experience(), `Ajouter Experience`).subscribe(result => {
      if (result) {
        this.update.next(true);
      }
    });
  ...
  edit(o: Experience) {
    this.openDialog(o, `Modifier Experience`).subscribe((result: Experience) => {
      if (result) {
        this.update.next(true);
      }
    });
  ...
  detail(o: Experience) {
    this.openDialog(o, `Détail Experience`).subscribe((result: Experience) => {
      if (result) {
        this.update.next(true);
      }
    });
  ...
  async delete(id: number) {
    const r = await this.mydialog.openDialog('Experience').toPromise();
    if (r === 'ok') {
      const sub = this.uow.experiences.delete(id).subscribe(() => this.update.next(true));
  ...
  displayImage(urlImage: string) {
    ...
    return `${this.url}/experiences/${urlImage.replace(';', '')}`;
  }
  ...
  //check box
  //
  isSelected(row: Experience): boolean {
    return this.selectedList.find(e => e.id === row.id) ? true : false;
  }

  check(row: Experience) {
  ...
  async deleteList() {
    ...
    if (r === 'ok') {
      const sub = this.uow.experiences.deleteRangeByIds(this.selectedList.map(e => e.id)).subscribe(() => {
        this.selectedList = [];
        this.update.next(true);
      });
