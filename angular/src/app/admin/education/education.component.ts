...
export class EducationComponent implements OnInit, OnDestroy {
  ...
  add() {
    this.openDialog(new Education(), `Ajouter Education`).subscribe(result => {
      if (result) {
        this.update.next(true);
      }
    });
  ...
  edit(o: Education) {
    this.openDialog(o, `Modifier Education`).subscribe((result: Education) => {
      if (result) {
        this.update.next(true);
      }
    });
  ...
  detail(o: Education) {
    this.openDialog(o, `Détail Education`).subscribe((result: Education) => {
      if (result) {
        this.update.next(true);
      }
    });
  ...
  async delete(id: number) {
    const r = await this.mydialog.openDialog('Education').toPromise();
    if (r === 'ok') {
      const sub = this.uow.educations.delete(id).subscribe(() => this.update.next(true));
  ...
  displayImage(urlImage: string) {
    ...
    return `${this.url}/educations/${urlImage.replace(';', '')}`;
  }
  ...
  //check box
  //
  isSelected(row: Education): boolean {
  ...
  async deleteList() {
    ...
    if (r === 'ok') {
      const sub = this.uow.educations.deleteRangeByIds(this.selectedList.map(e => e.id)).subscribe(() => {
        this.selectedList = [];
        this.update.next(true);
      });
